...
import { beginInstagramOAuth, disconnectInstagram, getInstagramStatus, loadInstagramMedia, syncInstagramComments } from "@/app/sorteios/actions";
...
export function InstagramImport({ locked, onImported }: { locked: boolean; onImported: (value: { entries: ParticipationEntry[]; mediaId: string; permalink: string; integrationId: string; syncedAt: string }) => void }) {
  const [status, setStatus] = useState<{ connected: boolean; username?: string; expiresAt?: string | null } | null>(null);
  const [media, setMedia] = useState<Media[]>([]);
  const [selected, setSelected] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  useEffect(() => { getInstagramStatus().then(setStatus).catch((value) => setError(value instanceof Error ? value.message : "Falha ao consultar a integracao.")); }, []);
  ...
  function disconnect() {
    ...
    setError(null);
    startTransition(async () => {
      try { await disconnectInstagram(); setStatus({ connected: false }); setMedia([]); setSelected(""); }
