...
export function AdminStatCard({ label, value, hint, tooltip, tone = 'default', href, actionLabel = 'Ver detalhes', icon: Icon, compact = false }: AdminStatCardProps) {
  const content = <>
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        <p className={cx('font-medium text-slate-400', compact ? 'text-[11px] leading-4' : 'text-xs uppercase tracking-[0.2em]')} title={tooltip}>{label}</p>
  ...
  const labelText = `${label}${tooltip ? `. ${tooltip}` : ''}`;
  return href
    ? <Link href={href} className={className} title={tooltip} aria-label={labelText}>{content}</Link>
    : <article className={className} title={tooltip}>{content}</article>;
}
