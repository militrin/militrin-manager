          ...
          {!data.organization ? <AdminEmptyState title="Nenhuma organização vinculada à sua conta" description="Fale com um administrador para vincular sua conta a uma organização." /> : !data.events.length ? <AdminEmptyState title="Nenhum evento cadastrado" description="Cadastre e ative um evento para liberar o painel operacional." /> : <>
            ...
            {sectionAccess.inventory && data.hasData ? <AdminSection compact title="Estoque de camisetas">
              <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                ...
                <AdminStatCard compact label="Saldo líquido" value={metric('shirts_available').value} href={href('shirts_available')} icon={Warehouse} hint="Recebidas − entregues − reservadas. Sobra de um tamanho não cobre falta de outro." tooltip="Saldo líquido global. Falta encomendar soma só as variantes com demanda acima do físico." />
                <AdminStatCard compact label="Falta encomendar" value={metric('shirts_deficit').value} href={href('shirts_deficit')} icon={TriangleAlert} tone={metric('shirts_deficit').value ? 'warning' : 'default'} hint="Demanda a encomendar por variante. No modo sob encomenda isso é operacional." />
              </div>
