...
export default async function ImportReviewQueuePage({ searchParams }: { searchParams: Promise<{ batchId?: string }> }) {
  async function submitReview(formData: FormData) {
    'use server';
    await resolveImportReviewAction(formData);
  }
    ...
    {(rows ?? []).map((row) => {
      ...
      return <article key={row.id} className="rounded-3xl border border-amber-700/50 bg-slate-900/80 p-5">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <div className="space-y-3"><h3 className="font-medium">Candidatos encontrados</h3>{candidates.map((candidate) => <div key={`${candidate.registration_contact_id}-${candidate.reason}`} className="rounded-2xl border border-slate-700 p-4 text-sm"><p className="font-semibold">{candidate.full_name || 'Cadastro sem nome'}</p><p className="text-xs text-emerald-300">{labels[candidate.reason ?? ''] ?? candidate.reason}</p><p className="mt-2 text-slate-400">CPF: {maskCpf(candidate.cpf ?? '')} · E-mail: {candidate.email || '-'}</p><form action={submitReview} className="mt-3"><input type="hidden" name="row_id" value={row.id}/><input type="hidden" name="decision" value="link_existing"/><input type="hidden" name="registration_contact_id" value={candidate.registration_contact_id}/><button className="rounded-xl bg-emerald-400 px-4 py-2 font-semibold text-emerald-950">É a mesma pessoa / vincular</button></form></div>)}</div>
        </div>
        <div className="mt-4 rounded-2xl bg-slate-950/60 p-3 text-xs text-slate-400">Vincular reutiliza o cadastro e preserva sua conta; criar novo mantém a linha separada; ignorar encerra a linha sem criar pedido, item ou ingresso. A decisão fica registrada em auditoria.</div>
        <div className="mt-4 flex flex-wrap gap-3"><form action={submitReview}><input type="hidden" name="row_id" value={row.id}/><input type="hidden" name="decision" value="create_new"/><button className="rounded-xl border border-sky-600 px-4 py-2 text-sm text-sky-200">É outra pessoa / criar novo</button></form><form action={submitReview}><input type="hidden" name="row_id" value={row.id}/><input type="hidden" name="decision" value="ignore"/><button className="rounded-xl border border-rose-800 px-4 py-2 text-sm text-rose-300">Ignorar linha</button></form><Link href={`/importacoes?batchId=${row.import_batch_id}`} className="rounded-xl border border-slate-700 px-4 py-2 text-sm">Abrir e reprocessar batch</Link></div>
      </article>;
    })}
