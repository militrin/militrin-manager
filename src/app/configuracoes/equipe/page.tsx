...
export default async function TeamSettingsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const activeTab = TABS.some(([code]) => code === params.tab) ? (params.tab as (typeof TABS)[number][0]) : 'team';

  const supabase = await createServerSupabaseClient();

  const [{ data: rolesData, error: rolesError }, canEditPermissions] = await Promise.all([
  ...
  const { data: teamRows, error: teamError } =
    activeTab === 'team'
      ? await supabase.rpc('list_admin_team', { p_search: q || null, p_role_name: role || null, p_status: status || null })
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 lg:flex-row">
        ...
        <div className="flex-1 space-y-6">
          <AdminPageHeader
            ...
            subtitle="Gerencie membros da equipe e as permissoes padrao de cada funcao"
            breadcrumbs={[{ label: 'Início', href: '/painel' }, { label: 'Configurações', href: '/configuracao' }, { label: 'Equipe' }]}
            backHref="/configuracao"
          />

          <nav className="flex flex-wrap gap-2" aria-label="Áreas de equipe e permissões">
            {TABS.map(([code, label]) => (
              <Link
                key={code}
                href={`/painel/configuracoes/equipe?tab=${code}`}
                className={`rounded-xl border px-3 py-1.5 text-xs ${activeTab === code ? 'border-emerald-400 bg-emerald-500/10 text-emerald-200' : 'border-slate-700 text-slate-300'}`}
              >
                {label}
              </Link>
            ))}
          ...
          {activeTab === 'team' ? (
            <>
              ...
              <AdminSection
                ...
                actions={canEditPermissions ? <AddTeamMemberButton roleOptions={roleOptions} /> : null}
              >
                {rows.length === 0 ? (
            ...
            <AdminSection title="Funções e permissões" description="Cada função define as permissões que seus usuários herdam por padrão -- overrides individuais continuam tendo precedência">
              {roles.length === 0 ? (
                <AdminEmptyState title="Nenhuma função encontrada" description="Verifique se as migrations de RBAC foram aplicadas no banco." />
              ) : (
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {roles.map((item: Record<string, unknown>) => {
                    const id = String(item.id);
                    const name = String(item.name ?? '');
                    const isOwnerRole = name === 'Owner';
                    return (
                      <div key={id} className="flex flex-col justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                        <div>
                          <p className="font-semibold text-white">{name}</p>
                          <p className="mt-1 text-xs text-slate-400">{String(item.description ?? 'Sem descrição.')}</p>
                          {isOwnerRole ? (
                            <p className="mt-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-2 py-1 text-[11px] text-amber-200">
                              Acesso total, não pode ser limitado
                            </p>
                          ) : null}
                        </div>
                        {canEditPermissions ? (
                          <Link
                            href={`/painel/configuracoes/equipe/funcoes/${id}`}
                            className="inline-flex items-center justify-center rounded-lg border border-slate-700 px-3 py-2 text-xs text-slate-200"
                          >
                            {isOwnerRole ? 'Ver função' : 'Editar permissões'}
                          </Link>
                        ) : null}
                      </div>
                    );
