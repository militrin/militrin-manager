...
export function HomePendingPurchase({
  ...
  return (
    <section className="flex h-full flex-col justify-between rounded-[1.25rem] border border-amber-500/30 bg-gradient-to-br from-slate-900/90 to-amber-950/20 p-3 shadow-lg shadow-black/10 sm:rounded-[1.5rem] sm:p-4">
      <div>
        ...
        <p className={cx('mt-1 text-xl sm:text-2xl', militrinType.money)}>{money(amount)}</p>
      </div>
      <div className="mt-3 flex items-center gap-3">
        <MilitrinLinkButton href={`/minha-conta/compras/${orderId}`} size="sm" className="min-w-0 flex-1">
          Continuar pagamento
        </MilitrinLinkButton>
