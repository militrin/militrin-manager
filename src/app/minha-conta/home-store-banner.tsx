...
export function HomeStoreBanner({
  ...
  return (
    ...
    >
      <div className="relative z-10 flex min-w-0 flex-1 items-center gap-2.5 px-3 py-2.5 sm:gap-3 sm:p-4">
      ...
      {visuals.length > 0 ? (
        <div aria-hidden className="relative flex w-[42%] max-w-[13.5rem] items-center justify-end gap-1.5 pr-2 sm:w-[38%] sm:max-w-xs sm:gap-2 sm:pr-4">
          {visuals.slice(0, 3).map((item, index) => (
            <div
              key={`${item.id}-${item.imageUrl}`}
              className="relative h-14 w-11 overflow-hidden rounded-lg border border-white/10 bg-slate-900 shadow-lg sm:h-20 sm:w-16 sm:rounded-xl"
