...
import { createAccountStoreOrderAction, type StoreCartLine } from '@/lib/store/actions';
...
export function CartPageClient() {
  ...
  return (
    <div className="space-y-4">
      {groups.map((group) => (
        <EventCartGroup
          key={group.eventId ?? '__global__'}
          eventId={group.eventId}
          eventName={group.eventName}
          lines={group.lines}
          onLeavingCheckout={() => setLeavingCheckout(true)}
        />
      ))}
    </div>
  );
