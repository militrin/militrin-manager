-- 132_manual_issue_shirt_stock_semantics.sql
-- Corrige a checagem de camiseta das duas RPCs de emissao manual (131) para
-- usar a MESMA semantica ja usada por create_registration (checkout
-- publico, migration 125): (1) so exige/valida camiseta quando o evento tem
-- um item ativo do tipo 'shirt' em event_kit_items; (2) so bloqueia por
-- falta de estoque e so incrementa reserved_quantity quando o evento tem
-- events.limit_shirt_selection_to_stock = true; quando o evento permite
-- "livre para encomenda" (false), o tipo/tamanho e apenas registrado no
-- ingresso, sem checar nem reservar estoque fisico. A versao anterior (131)
-- sempre exigia uma linha de shirt_inventory com saldo -- o que quebraria a
-- emissao manual em qualquer evento configurado como "sob encomenda".
-- Mesma assinatura da 131, so precisa de create or replace.
