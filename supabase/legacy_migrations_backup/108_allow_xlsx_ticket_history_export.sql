-- 108_allow_xlsx_ticket_history_export.sql
-- Estende a auditoria protegida da exportacao para o formato XLSX.
begin;
...
declare
  ...
  v_definition text;
begin
  if v_signature is null then raise exception 'Funcao 107 record_ticket_history_export nao encontrada.'; end if;
  if exists(
    select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='record_ticket_history_export' and p.oid<>v_signature::oid
  ...
  if position('ticket_history_exported' in v_definition)=0
    ...
    raise exception 'Definicao ativa de record_ticket_history_export diverge do contrato protegido 107/108.';
  end if;
end;
$migration$;
...
begin
  ...
  if position('security definer' in v_definition)=0
     or position('set search_path to ''public'', ''pg_temp''' in v_definition)=0
     or position('auth.uid()' in v_definition)=0
     or position('now()' in v_definition)=0
     or position('v_formatnotin(''pdf'',''csv'',''xlsx'')' in v_normalized)=0
     or position('''actor_user_id'',v_actor' in v_normalized)=0
     or position('audit_logs(id,actor,' in v_normalized)>0
     or position('service_role' in v_definition)>0 then
    raise exception 'Validacao final da funcao 108 falhou.';
  end if;
end;
$migration$;
