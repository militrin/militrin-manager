-- 030_fix_checkout_delete_cleanup.sql
-- Corrige DELETE sem WHERE no fluxo de checkout multi-ingressos.
...
begin
  for v_fn in
    ...
    where n.nspname = 'public'
      and p.proname in (
        'create_multi_ticket_order_checkout',
        'create_multi_ticket_order_checkout_legacy',
  ...
  loop
    ...
    if v_def ~* 'delete[[:space:]]+from[[:space:]]+[^;]+;'
       and v_def !~* 'delete[[:space:]]+from[[:space:]]+[^;]+[[:space:]]where[[:space:]]+' then
      raise warning '[checkout-rpc-audit] DELETE sem WHERE detectado em %.%', v_fn.schema_name, v_fn.proname;
    else
      raise notice '[checkout-rpc-audit] OK %.%', v_fn.schema_name, v_fn.proname;
    end if;
  end loop;
end;
$$;
...
begin
  ...
  v_tmp_rel := to_regclass('pg_temp.tmp_inventory_checkout_boost');

  if v_tmp_rel is not null then
