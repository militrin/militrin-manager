...
-- Fase 1 Asaas -- fecha o risco encontrado na auditoria: o UPSERT em
-- `tickets` dentro de `confirm_order_item_and_issue_ticket` sobrescrevia
-- incondicionalmente `status='active', cancelled_at=null, used_at=null` no
-- braco ON CONFLICT, mesmo quando o ticket ja existente estava 'cancelled'
-- (cancelamento administrativo, ex.: public.owner_cancel_ticket) ou 'used'
-- (check-in ja feito). Um webhook tardio/duplicado reprocessando a mesma
-- confirmacao de pagamento reativava o ticket silenciosamente.
--
-- Regra final (documentada aqui e no relatorio da Fase 1):
--   - retry de confirmacao do mesmo pagamento pago -> idempotente (mesma
--     linha, nenhuma duplicata, nenhuma mudanca de status);
--   - ticket ja ativo -> mantem 'active' (linkage columns podem ser
--     re-sincronizadas, status nunca muda no conflito);
--   - ticket usado -> nao altera (status/used_at preservados);
--   - ticket cancelado por decisao administrativa/estorno -> o conflito NUNCA
--     reescreve status/cancelled_at/used_at; a funcao detecta o caso,
--     registra 'ticket_reactivation_blocked' em audit_logs e retorna NULL
--     (mesmo contrato de retorno ja usado para itens de produto -- callers
--     existentes ja tratam NULL como "nenhum ticket novo emitido").
