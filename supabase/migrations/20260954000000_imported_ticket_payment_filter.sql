-- Classificacao de pagamento da listagem admin para ingressos importados.
-- Nao altera dados: leitura pura. Nao grava payment ficticio.
-- Nao edita a migration 53 ja aplicada.
--
-- Identificacao canonica de legado:
--   orders.buyer_type = 'imported_holder'
--   AND orders.import_batch_id IS NOT NULL
-- (CHECK orders_buyer_ownership_check). Nao usa nome/e-mail.
--
-- Ingresso nativo: payments.payment_status via orders.payment_id.
-- Ingresso importado materializado: PAGO comercialmente, inclusive sem
-- payment moderno. Cancelamento operacional do ingresso/pedido NAO vira
-- estorno financeiro. Situacao: Ativos exclui o cancelado; Pagamento: Pago
-- + Situacao: Todos ainda o encontra, salvo cancelled/refunded/expired.

begin;

drop function if exists public.ticket_admin_payment_class(text, uuid, text, text, text);

create or replace function public.ticket_admin_payment_class(
  p_buyer_type text,
...
as $$
  select case
    when lower(trim(coalesce(p_buyer_type, ''))) = 'imported_holder'
      and p_import_batch_id is not null then
      case
        when lower(trim(coalesce(p_payment_status, ''))) in ('cancelled', 'refunded', 'expired')
        then 'cancelado'
        else 'pago'
      end
    when lower(trim(coalesce(p_payment_status, ''))) = 'paid' then 'pago'
    when lower(trim(coalesce(p_payment_status, ''))) = 'pending' then 'pendente'
    when lower(trim(coalesce(p_payment_status, ''))) in ('cancelled', 'refunded', 'expired') then 'cancelado'
    else null
  end;
$$;

revoke all on function public.ticket_admin_payment_class(text, uuid, text) from public, anon;
grant execute on function public.ticket_admin_payment_class(text, uuid, text) to authenticated, service_role;
