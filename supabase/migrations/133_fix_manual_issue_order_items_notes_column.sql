-- 133_fix_manual_issue_order_items_notes_column.sql
-- Corrige erro real em producao: "column notes of relation order_items does
-- not exist". As duas RPCs de emissao manual (heranca das migrations
-- 089/090, mantida sem essa checagem nas migrations 131/132) tentavam
-- gravar p_notes na coluna order_items.notes, que nunca existiu nessa
-- tabela (confirmado em 023_order_items_multi_ticket.sql -- o checkout
-- publico, create_multi_ticket_order_checkout, nunca grava notes ali).
-- A observacao do admin passa a ser gravada em participants.notes (que
-- existe desde 001_initial_schema.sql) quando ha um cadastro vinculado, e
-- em audit_logs.details nos dois casos, para nao se perder quando o
-- ingresso e "unassigned" (sem participante).
