-- Central de Convites: catalogo de permissoes, leitura agregada e jobs de reenvio.
-- Nao duplica o ciclo de convite: estados de UI sao derivados das tabelas atuais.
-- Nao altera a migration local conflitante de importacao legado.

begin;

insert into public.admin_permissions (code, name, description, module, sort_order, is_active)
values
  ('invites.view', 'Ver Central de Convites', 'Visualiza o ciclo de convite e primeiro acesso da organização', 'invites', 10, true),
  ('invites.resend', 'Reenviar convite', 'Reenvia convite de primeiro acesso individual sem criar Auth duplicado', 'invites', 20, true),
  ('invites.bulk_resend', 'Reenviar convites em massa', 'Dispara reenvio controlado de convites expirados/falhos após confirmação', 'invites', 30, true)
...
join public.admin_permissions permission on permission.code in ('invites.view', 'invites.resend')
...
select role.id, permission.id
from public.admin_roles role
join public.admin_permissions permission on permission.code = 'invites.bulk_resend'
...
where p.code in ('invites.view', 'invites.resend', 'invites.bulk_resend')
on conflict do nothing;

create table if not exists public.account_invite_center_jobs (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  created_by uuid not null,
  kind text not null check (kind = 'resend_expired'),
  status text not null check (status = any (array['preview','pending','processing','completed','completed_with_failures','cancelled'])),
...
create table if not exists public.account_invite_center_job_items (
  id uuid primary key default gen_random_uuid(),
  job_id uuid not null references public.account_invite_center_jobs(id) on delete cascade,
  organization_id uuid not null,
  invite_id uuid,
  registration_contact_id uuid,
  email_normalized text not null,
  status text not null check (status = any (array['pending','processing','sent','skipped','failed'])),
  skip_reason text,
  error_safe text,
...
create index if not exists idx_invite_center_jobs_org_status
  on public.account_invite_center_jobs (organization_id, status, created_at desc);
create index if not exists idx_invite_center_job_items_job_status
  on public.account_invite_center_job_items (job_id, status);

alter table public.account_invite_center_jobs enable row level security;
alter table public.account_invite_center_job_items enable row level security;
revoke all on table public.account_invite_center_jobs from public, anon, authenticated;
revoke all on table public.account_invite_center_job_items from public, anon, authenticated;

create or replace function public.invite_center_mask_email(p_email text)
returns text
language sql
immutable
as $$
...
as $$
  select case
    ...
    when p_invite_status = 'claimed'
      and coalesce(p_must_complete_profile, false) is false
      and coalesce(p_must_change_password, false) is false
      and coalesce(p_cadastral_incomplete, false) is false
      and (p_activation_completed_at is not null or coalesce(p_account_status, '') = 'active')
      then 'concluido'
    when p_invite_status = 'claimed' then 'cadastro_pendente'
    when p_invite_status = 'pending' and p_expires_at is not null and p_expires_at <= now() then 'expirado'
  ...
  sorted as (
    ...
    order by
      case coalesce(p_sort, 'attention')
        ...
        when 'expiring' then extract(epoch from expires_at)
        else null
      end desc nulls last,
    ...
    'rows', coalesce((
      ...
      from (
        select jsonb_build_object(
          ...
          'can_resend', s.ui_status in ('pendente', 'expirado', 'falha', 'nao_enviado'),
          'people', coalesce((
...
create or replace function public.preview_invite_center_bulk_resend()
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
declare
  ...
  v_rows jsonb;
begin
  if auth.uid() is null or not public.current_user_has_permission('invites.bulk_resend') then
    raise exception 'Sem permissao.';
  end if;
  v_list := public.list_invite_center(null, null, 'reenvio', 'all', null, 'attention', 500, 0);
  select coalesce(jsonb_agg(item), '[]'::jsonb)
    into v_rows
  from jsonb_array_elements(v_list->'rows') item;

  return jsonb_build_object(
    'expired_or_failed', jsonb_array_length(v_rows),
    'unique_emails', jsonb_array_length(v_rows),
    'conflicts', 0,
    'already_active', 0,
    'sample_masked', (
      select coalesce(jsonb_agg(item->>'email_masked'), '[]'::jsonb)
      from (
        select item
    ...
    'sent_last_24h', coalesce((v_list->>'sent_last_24h')::int, 0),
    'batch_size', 5,
    'sends_nothing', true
  );
end;
...
create or replace function public.start_invite_center_bulk_resend()
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
declare
  ...
  v_org uuid;
  v_list jsonb;
  v_job public.account_invite_center_jobs%rowtype;
  v_item jsonb;
  v_count int := 0;
begin
  if v_actor is null or not public.current_user_has_permission('invites.bulk_resend') then
    raise exception 'Sem permissao.';
  end if;
  select o.id into v_org
  from public.organizations o
  ...
  order by o.created_at
  limit 1;
  if v_org is null then raise exception 'Organizacao invalida.'; end if;

  if exists (
    select 1 from public.account_invite_center_jobs
    where organization_id = v_org and status in ('pending', 'processing')
  ) then
    raise exception 'Ja existe um reenvio em massa em andamento.';
  end if;

  v_list := public.list_invite_center(null, null, 'reenvio', 'all', null, 'attention', 500, 0);

  insert into public.account_invite_center_jobs (
    organization_id, created_by, kind, status, started_at
  ) values (
    v_org, v_actor, 'resend_expired', 'pending', now()
  ) returning * into v_job;

  for v_item in
    select item from jsonb_array_elements(v_list->'rows') item
    where item->>'status' in ('expirado', 'falha')
  loop
    insert into public.account_invite_center_job_items (
      job_id, organization_id, invite_id, registration_contact_id, email_normalized, status
    ) values (
      v_job.id,
      v_org,
      nullif(v_item->>'invite_id', '')::uuid,
      nullif(v_item->>'principal_contact_id', '')::uuid,
      coalesce(v_item->>'row_key', v_item->>'principal_contact_id'),
      'pending'
    )
  ...
  update public.account_invite_center_jobs
  set total_count = v_count, unique_email_count = v_count, status = case when v_count = 0 then 'completed' else 'pending' end,
      finished_at = case when v_count = 0 then now() else null end
  where id = v_job.id;

  insert into public.audit_logs(action, entity_type, entity_id, event_id, details)
  values (
    'account_invite_bulk_resend_started',
    'account_invite_center_jobs',
    v_job.id,
    null,
    jsonb_build_object('actor_user_id', v_actor, 'total_count', v_count)
  );

  return jsonb_build_object('job_id', v_job.id, 'total_count', v_count);
end;
$$;

create or replace function public.claim_invite_center_bulk_items(p_job_id uuid, p_limit integer default 5)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  select * into v_job from public.account_invite_center_jobs where id = p_job_id for update;
  if not found then raise exception 'Job invalido.'; end if;
  if v_job.status in ('completed', 'completed_with_failures', 'cancelled') then
    return jsonb_build_object('job', to_jsonb(v_job), 'items', '[]'::jsonb);
  end if;
  ...
  with claimed as (
    ...
    update public.account_invite_center_job_items i
    set status = 'processing', claimed_at = now(), attempt_count = i.attempt_count + 1
    from claimed
    where i.id = claimed.id
    returning i.id, i.invite_id, i.registration_contact_id, i.email_normalized
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    'id', updated.id,
    'invite_id', updated.invite_id,
    'registration_contact_id', updated.registration_contact_id,
    'email_normalized', updated.email_normalized
  )), '[]'::jsonb) into v_items
  from updated;

  return jsonb_build_object(
    'job', (select to_jsonb(j) from public.account_invite_center_jobs j where j.id = p_job_id),
    'items', v_items
  );
end;
$$;

create or replace function public.finish_invite_center_bulk_item(
  p_item_id uuid,
  p_status text,
...
begin
  ...
  update public.account_invite_center_job_items
  set status = p_status,
      error_safe = left(coalesce(p_error_safe, ''), 200),
      sent_at = case when p_status = 'sent' then now() else sent_at end
  where id = p_item_id and status = 'processing'
  returning job_id into v_job_id;
  if v_job_id is null then raise exception 'Item invalido.'; end if;

  select
    count(*) filter (where status = 'failed'),
    count(*) filter (where status in ('pending', 'processing'))
  into v_failed, v_pending
  from public.account_invite_center_job_items
  where job_id = v_job_id;

  update public.account_invite_center_jobs j
  set processed_count = (select count(*) from public.account_invite_center_job_items i where i.job_id = j.id and i.status in ('sent','skipped','failed')),
      sent_count = (select count(*) from public.account_invite_center_job_items i where i.job_id = j.id and i.status = 'sent'),
      failed_count = v_failed,
      skipped_count = (select count(*) from public.account_invite_center_job_items i where i.job_id = j.id and i.status = 'skipped'),
      status = case
        when v_pending > 0 then 'processing'
        when v_failed > 0 then 'completed_with_failures'
        else 'completed'
      end,
      finished_at = case when v_pending > 0 then null else now() end,
      last_error_safe = case when p_status = 'failed' then left(coalesce(p_error_safe, ''), 200) else j.last_error_safe end
  where j.id = v_job_id;

  return jsonb_build_object(
    'job', (select to_jsonb(j) from public.account_invite_center_jobs j where j.id = v_job_id)
  );
end;
$$;

create or replace function public.get_invite_center_bulk_progress(p_job_id uuid default null)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if p_job_id is not null then
    select * into v_job
    from public.account_invite_center_jobs
    where id = p_job_id and organization_id = v_org;
  else
    select * into v_job
    from public.account_invite_center_jobs
  ...
  if v_job.id is null then
    return jsonb_build_object('job', null, 'sent_last_24h', (
      select count(*)::int from public.account_invite_center_job_items i
      where i.organization_id = v_org and i.status = 'sent' and i.sent_at > now() - interval '24 hours'
    ));
  end if;
  ...
  return jsonb_build_object(
    'job', jsonb_build_object(
      ...
      'unique_email_count', v_job.unique_email_count,
      'processed_count', v_job.processed_count,
      'sent_count', v_job.sent_count,
      'failed_count', v_job.failed_count,
      'skipped_count', v_job.skipped_count,
      'last_error_safe', v_job.last_error_safe,
...
create index if not exists idx_registration_contacts_org_email_lower
  on public.registration_contacts (organization_id, (lower(trim(email))))
  where nullif(trim(email), '') is not null;
create index if not exists idx_participant_account_invites_org_email_lower
  on public.participant_account_invites (organization_id, (lower(trim(email))));
create index if not exists idx_tickets_org_intended_owner
  on public.tickets (organization_id, intended_owner_contact_id)
  where intended_owner_contact_id is not null;

revoke all on function public.invite_center_mask_email(text) from public, anon;
revoke all on function public.invite_center_ui_status(boolean, text, timestamptz, text, boolean, boolean, timestamptz, boolean, text) from public, anon;
...
revoke all on function public.preview_invite_center_bulk_resend() from public, anon;
revoke all on function public.start_invite_center_bulk_resend() from public, anon;
revoke all on function public.claim_invite_center_bulk_items(uuid, integer) from public, anon;
revoke all on function public.finish_invite_center_bulk_item(uuid, text, text) from public, anon;
revoke all on function public.get_invite_center_bulk_progress(uuid) from public, anon;

grant execute on function public.invite_center_mask_email(text) to authenticated, service_role;
grant execute on function public.invite_center_ui_status(boolean, text, timestamptz, text, boolean, boolean, timestamptz, boolean, text) to authenticated, service_role;
...
grant execute on function public.preview_invite_center_bulk_resend() to authenticated, service_role;
grant execute on function public.start_invite_center_bulk_resend() to authenticated, service_role;
grant execute on function public.claim_invite_center_bulk_items(uuid, integer) to authenticated, service_role;
grant execute on function public.finish_invite_center_bulk_item(uuid, text, text) to authenticated, service_role;
grant execute on function public.get_invite_center_bulk_progress(uuid) to authenticated, service_role;
