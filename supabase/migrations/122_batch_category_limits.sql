-- 122_batch_category_limits.sql
-- Substitui o limite unico por lote (registration_batches.max_confirmed_registrations)
-- por um limite configuravel por (lote, categoria), guardado em
-- registration_batch_prices.max_confirmed_registrations. Uma categoria pode
-- esgotar dentro de um lote enquanto outra categoria do mesmo lote ainda tem
-- vaga -- nesse caso so a categoria esgotada e bloqueada; o lote so avanca
-- para o proximo quando TODAS as categorias ativas nele estiverem esgotadas.
--
-- Escopo tocado com seguranca (corpo estatico, verificavel a partir das
-- migrations, sem EXECUTE dinamico): get_registration_batches,
-- get_event_ticket_categories, advance_registration_batch_if_needed,
-- upsert_registration_batch_prices, create_registration_batch_with_prices,
-- update_registration_batch_with_prices, get_registration_pricing_preview,
-- create_registration, confirm_registration_payment.
--
-- NAO tocamos em create_multi_ticket_order_checkout (corpo remendado com
-- EXECUTE dinamico em varias migrations, nao verificavel na integra) -- mas
-- ela resolve lote/preco chamando get_registration_pricing_preview, entao
-- herda a nova checagem por categoria automaticamente, sem precisar editar
-- o corpo dela.

begin;

-- ============================================================
-- 1. Coluna de limite por (lote, categoria).
-- ============================================================

alter table public.registration_batch_prices add column if not exists max_confirmed_registrations integer;

update public.registration_batch_prices rbp
set max_confirmed_registrations = rb.max_confirmed_registrations
from public.registration_batches rb
where rb.id = rbp.batch_id
  and rbp.max_confirmed_registrations is null;

alter table public.registration_batch_prices alter column max_confirmed_registrations set not null;

alter table public.registration_batch_prices drop constraint if exists registration_batch_prices_limit_positive;
alter table public.registration_batch_prices add constraint registration_batch_prices_limit_positive check (max_confirmed_registrations > 0);

-- ============================================================
-- 2. advance_registration_batch_if_needed: so avanca quando TODAS as
--    categorias ativas do lote atual estiverem esgotadas.
-- ============================================================
...
-- ============================================================
-- 3. upsert_registration_batch_prices: guarda o limite por categoria.
-- ============================================================
...
begin
  ...
  loop
    ...
    if v_max_confirmed is null or v_max_confirmed <= 0 then
      raise exception 'Categoria ativa no lote precisa de um limite de confirmados maior que zero.';
    end if;
...
--    removem p_max_confirmed_registrations (agora e por categoria, dentro de
--    p_prices) -- o limite do lote inteiro (mantido so como soma agregada,
--    sem uso em nenhuma checagem) e calculado a partir das categorias ativas.
-- ============================================================

drop function if exists public.create_registration_batch_with_prices(uuid, text, integer, timestamptz, timestamptz, boolean, jsonb);
...
begin
  ...
  loop
    ...
    if v_max_confirmed is null or v_max_confirmed <= 0 then
      raise exception 'Toda categoria ativa deve possuir um limite de confirmados maior que zero.';
    end if;

    v_enabled_count := v_enabled_count + 1;
    v_sum_max_confirmed := v_sum_max_confirmed + v_max_confirmed;
...
drop function if exists public.update_registration_batch_with_prices(uuid, uuid, text, integer, timestamptz, timestamptz, boolean, jsonb);
...
-- ============================================================
-- 5. get_registration_batches: uma linha por (lote, categoria).
-- ============================================================

drop function if exists public.get_registration_batches(uuid);
...
begin
  ...
  select
    ...
    greatest(rbp.max_confirmed_registrations - coalesce(c.confirmed_count, 0), 0)::integer as remaining_slots,
    batch.created_at,
    batch.updated_at
...
-- ============================================================
-- 6. get_event_ticket_categories: expoe o lote/preco atual de cada categoria
--    (primeiro lote ativo, em ordem de sequencia, que ainda tem vaga para
--    aquela categoria) -- usado na tela de escolha de categoria do publico,
--    para mostrar "Categoria - Nome do Lote - R$ valor" sem expor vagas.
-- ============================================================

drop function if exists public.get_event_ticket_categories(uuid);
...
-- ============================================================
-- 7. get_registration_pricing_preview: gate por (lote, categoria) em vez de
--    lote inteiro. create_multi_ticket_order_checkout chama esta funcao para
--    resolver lote/preco, entao herda a checagem por categoria sem precisar
--    editar o corpo dela.
-- ============================================================
...
begin
  ...
  if v_batch_category_limit is null then
  ...
  if v_confirmed_count >= v_batch_category_limit then
  ...
  if coalesce(trim(p_coupon_code), '') <> '' then
    ...
    select
      ...
      v_discount,
      v_final,
      greatest(v_batch_category_limit - v_confirmed_count, 0)::integer,
...
-- ============================================================
-- 8. create_registration: gate por (lote, categoria). Reproduz o corpo
--    integral da 121 (unica definicao ativa) com essa checagem trocada.
-- ============================================================
...
-- ============================================================
-- 9. confirm_registration_payment: gate por (lote, categoria) na
--    reconfirmacao de limite (protege contra corrida entre reserva e
...
begin
  ...
  if v_participant.batch_id is not null and v_participant.ticket_category_id is not null then
    ...
    if found then
      ...
      if v_batch_category_limit is not null then
        select count(*)::integer into v_confirmed_count
        from public.participants part
        join public.payments pay
          on pay.participant_id = part.id
