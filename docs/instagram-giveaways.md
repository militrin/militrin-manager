...
O callback `GET /api/instagram/oauth/callback` é público no middleware (como deauthorize/data-deletion). O `redirect_uri` canônico é exatamente `https://www.militrin.com.br/api/instagram/oauth/callback` (HTTPS, www, sem barra final). Authorize e token exchange usam o mesmo valor após `trim()`. O vínculo administrativo fica em `instagram_oauth_states` (nonce, admin, organização, expiração). O cookie `instagram_oauth_state` usa `Path=/`, HttpOnly, SameSite=Lax e é apagado após o callback. Sem sessão do WebView, o callback ainda conclui se o state server-side for válido.
