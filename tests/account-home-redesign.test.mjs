...
test('PIN da home no mobile reusa getMyPublicPin e some no desktop do card de acesso', async () => {
  ...
  assert.match(pin, /PIN da conta/);
  assert.match(pin, /Copiar PIN da conta/);
  assert.doesNotMatch(pin, /Meu PIN/);
  assert.doesNotMatch(pin, /Código do ingresso/);
  assert.match(layout, /<PublicPinCopy publicPin=\{publicPin\} \/>/);
  assert.doesNotMatch(layout, /compact/);
});

test('participante ve public_pin como PIN da conta, sem copy de PIN de ingresso', async () => {
  ...
  assert.match(holder, /placeholder="João da Silva"/);
  assert.doesNotMatch(holder, /placeholder="Código do ingresso"/);
  assert.doesNotMatch(holder, /PIN do usuário/);
  assert.match(actions, /Nenhum usuário encontrado para esse PIN da conta/);
  assert.doesNotMatch(actions, /código do ingresso/);
});
